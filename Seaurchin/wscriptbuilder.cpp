...
CWScriptBuilder::CWScriptBuilder()
{
    engine = 0;
    module = 0;
...
void CWScriptBuilder::SetIncludeCallback(WINCLUDECALLBACK_t callback, void *userParam)
{
    includeCallback = callback;
    callbackParam = userParam;
}

int CWScriptBuilder::StartNewModule(asIScriptEngine *inEngine, const char *moduleName)
{
...
asIScriptModule *CWScriptBuilder::GetModule()
{
    return module;
}

unsigned int CWScriptBuilder::GetSectionCount() const
{
    return (unsigned int)(includedScripts.size());
}

string CWScriptBuilder::GetSectionName(unsigned int idx) const
{
...
// Returns 0 if the section was not included because it had already been included before
// Returns <0 if there was an error
int CWScriptBuilder::AddSectionFromMemory(const char *sectionName, const char *scriptCode, unsigned int scriptLength, int lineOffset)
{
    if (IncludeIfNotAlreadyIncluded(ConvertUTF8ToUnicode(sectionName).c_str())) {
...
int CWScriptBuilder::BuildModule()
{
    return Build();
}

void CWScriptBuilder::DefineWord(const char *word)
{
    string sword = word;
...
int CWScriptBuilder::LoadScriptSection(const wchar_t *filename)
{
    ...
    if (f == 0) {
        ...
        wstring msg = L"Failed to open script file '" + GetAbsolutePath(scriptFile) + L"'";
        engine->WriteMessage(ConvertUnicodeToUTF8(filename).c_str(), 0, 0, asMSGTYPE_ERROR, ConvertUnicodeToUTF8(msg).c_str());
    ...
    if (c == 0 && len > 0) {
        // Write a message to the engine's message callback
        wstring msg = L"Failed to load script file '" + GetAbsolutePath(scriptFile) + L"'";
        engine->WriteMessage(ConvertUnicodeToUTF8(filename).c_str(), 0, 0, asMSGTYPE_ERROR, ConvertUnicodeToUTF8(msg).c_str());
        return -1;
    }

    // Process the script section even if it is zero length so that the name is registered
    return ProcessScriptSection(code.c_str(), (unsigned int)(code.length()), ConvertUnicodeToUTF8(filename).c_str(), 0);
}

int CWScriptBuilder::ProcessScriptSection(const char *script, unsigned int length, const char *sectionname, int lineOffset)
{
                ...
                if (t == asTC_IDENTIFIER) {
                    ...
                    if (token == "include") {
                        ...
                        if (t == asTC_VALUE && len > 2 && (modifiedScript[pos] == '"' || modifiedScript[pos] == '\'')) {
                            ...
                            includes.push_back(ConvertUTF8ToUnicode(includefile));
    ...
    if (includes.size() > 0) {
        ...
        if (includeCallback) {
            for (int n = 0; n < (int)includes.size(); n++) {
                int r = includeCallback(includes[n].c_str(), ConvertUTF8ToUnicode(sectionname).c_str(), this, callbackParam);
                if (r < 0)
                    return r;
            }
        } else {
            ...
            wstring path = ConvertUTF8ToUnicode(sectionname);
            size_t posOfSlash = path.find_last_of(L"/\\");
            ...
            for (int n = 0; n < (int)includes.size(); n++) {
                ...
                if (includes[n].find_first_of(L"/\\") != 0 &&
                    includes[n].find_first_of(L":") == string::npos) {
...
int CWScriptBuilder::SkipStatement(int pos)
{
    asUINT len = 0;
...
int CWScriptBuilder::ExcludeCode(int pos)
{
    asUINT len = 0;
    int nested = 0;
...
void CWScriptBuilder::OverwriteCode(int start, int len)
{
...
#if AS_PROCESS_METADATA == 1
int CWScriptBuilder::ExtractMetadataString(int pos, string &metadata)
{
    metadata = "";
...
int CWScriptBuilder::ExtractDeclaration(int pos, string &declaration, int &type)
{
    declaration = "";
    type = 0;
...
const char *CWScriptBuilder::GetMetadataStringForType(int typeId)
{
...
const char *CWScriptBuilder::GetMetadataStringForFunc(asIScriptFunction *func)
{
    if (func) {
...
const char *CWScriptBuilder::GetMetadataStringForVar(int varIdx)
{
...
const char *CWScriptBuilder::GetMetadataStringForTypeProperty(int typeId, int varIdx)
{
...
const char *CWScriptBuilder::GetMetadataStringForTypeMethod(int typeId, asIScriptFunction *method)
{
    if (method) {
        map<int, SClassMetadata>::iterator typeIt = classMetadataMap.find(typeId);
        if (typeIt == classMetadataMap.end()) return "";
